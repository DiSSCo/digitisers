...
import eu.dissco.digitisers.clients.ebi.openapi.auth.Authentication;
import eu.dissco.digitisers.clients.ebi.openapi.auth.HttpBasicAuth;
import eu.dissco.digitisers.clients.ebi.openapi.auth.HttpBearerAuth;
...
public class ApiClient {
    ...
    private void init() {
        ...
        setUserAgent("OpenAPI-Generator/all/java");
     ...
     * Set base path
     *
     * @param basePath Base path of the URL (e.g https://www.ebi.ac.uk/ebisearch/ws/rest
    ...
    public <T> T deserialize(Response response, Type returnType) throws ApiException {
        ...
        if (isJsonMime(contentType)) {
            respBody = this.fixEntityFieldsInResponseBody(respBody);
    ...
    //Francisco Quevedo. Entity field can not be parsed as it is an object rather than an array of WSFields
    private String fixEntityFieldsInResponseBody(String respBody){
        Gson gson = new Gson();
        JsonObject responseObj = gson.fromJson(respBody,JsonObject.class);
        if (responseObj.has("entries")){
            for (JsonElement entry:responseObj.getAsJsonArray("entries")) {
                if (entry.getAsJsonObject().has("fields")){
                    JsonObject fields = entry.getAsJsonObject().getAsJsonObject("fields");
                    JsonArray newFields = new JsonArray();
                    Set<Map.Entry<String, JsonElement>> items = fields.entrySet();
                    for (Map.Entry<String, JsonElement> item: items) {
                        JsonObject newField = new JsonObject();
                        newField.addProperty("id",item.getKey());
                        newField.add("values",item.getValue());
                        newFields.add(newField);
                    }
                    entry.getAsJsonObject().remove("fields");
                    entry.getAsJsonObject().add("fields",newFields);
                }
            }
        }
